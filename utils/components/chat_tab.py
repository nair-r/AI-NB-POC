...
def _response_card(prompt: str, response: str) -> str:
    return (
...
def _payload_summary_html(payload: dict) -> str:
    rows = [
...
def build_chat(state):
    ...
    def _build_payload():
        ...
        if n_slices > 1:
            ...
            return None, "No image or series selected. Pick one from the browser."
    ...
    def _on_send(_btn):
        ...
        payload_display.value = _payload_summary_html(payload)
        ...
        try:
            ...
            response_area.value = (
                _response_card(payload["prompt"], generated) + footer
            )
